...
// Golden-vector parity for the native crypto mirror. Loads the SAME
// crypto_vectors.json used by the Dart (crypto_vectors_test.dart) and Kotlin
...
final class CryptoVectorsTests: XCTestCase {
    ...
    // identity/test/crypto_vectors.json, so there's exactly one committed
    // copy, not a duplicate bundled into this package.
    ...
    func testSecretBoxDecryptVectors() {
        for v in cases("secretbox_decrypt") {
            let result = NativeCrypto.decryptSecretBox(
                base64Ciphertext: str(v["b64"]), key: bytes(v["keyB64"]))
            if (v["expectFail"] as? Bool) == true {
                XCTAssertNil(result, "\(str(v["id"])): \(str(v["desc"]))")
                continue
            }
            guard let data = result,
                  let json = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any] else {
                XCTFail("\(str(v["id"])): decryptSecretBox failed"); continue
            }
            let expected = v["expected"] as! [String: Any]
            for (k, want) in expected {
                XCTAssertEqual(json[k] as? String, want as? String, "\(str(v["id"])): field \(k)")
